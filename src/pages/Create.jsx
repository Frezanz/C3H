...
export default function Create() {
  ...
  const handleTagKeyDown = (event) => {
    ...
    if (event.key === 'Escape') { setTagInput(''); setShowTagInput(false); }
  };
  ...
  const saveDraft = () => {
    const draft = {
      type, title, body, tags, visibility, selectedGroup, passwordEnabled,
      selectedPeople: selectedPeople.map((person) => person?.User?.Id ?? person?.Id).filter(Boolean),
  ...
  return (
    <div className="mx-auto min-h-[calc(100vh-4rem)] w-full max-w-4xl px-0 py-6 sm:py-10">
      <div className="mb-5 flex items-center gap-3 px-1">
        <Link to="/" aria-label="Back to home" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-border bg-card text-muted-foreground shadow-xs transition hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.97]"><ApperIcon name="ArrowLeft" size={18} /></Link>
        <div className="min-w-0"><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Create</div><h1 className="mt-1 text-2xl font-heading font-semibold tracking-tight">Add something to C3H</h1></div>
      </div>
