...
function PasswordField({ id, label, value, onChange, show, toggle }) { return <div className="space-y-1"><Label htmlFor={id}>{label}</Label><div className="relative"><Input id={id} type={show ? 'text' : 'password'} value={value} onChange={onChange} required className="h-9 pr-10" /><button type="button" onClick={toggle} className="absolute inset-y-0 right-0 flex items-center pr-3 text-muted-foreground" tabIndex={-1} aria-label={show ? 'Hide password' : 'Show password'}><ApperIcon name={show ? 'EyeOff' : 'Eye'} size={16} /></button></div></div>; }
