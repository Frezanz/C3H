      ...
      <header className="sticky top-0 z-[var(--z-sticky)] border-b border-border/70 bg-background/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            {user?.Id && <Link to="/create" className="hidden items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground shadow-xs transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:inline-flex"><Icon name="Plus" size={16} /> Create</Link>}
          ...
          <aside className="fixed right-0 top-0 z-[calc(var(--z-sticky)+2)] flex h-dvh w-[min(50vw,380px)] max-w-[50vw] flex-col border-l border-border bg-background shadow-2xl max-sm:w-[88vw] max-sm:max-w-[88vw]" aria-label="C3H menu">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-5 sm:px-4">
              ...
              {user?.Id ? <nav className="space-y-1" aria-label="Account navigation">
                <NavLink to="/create" className={navClass}><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-primary-foreground"><Icon name="Plus" size={17} /></span><span className="flex-1">Create</span><Icon name="ChevronRight" size={15} className="text-muted-foreground" /></NavLink>
